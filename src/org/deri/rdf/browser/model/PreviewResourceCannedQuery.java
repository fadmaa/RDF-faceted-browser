...
public class PreviewResourceCannedQuery {
	...
	public PreviewResourceCannedQuery(InputStream properties)
			throws IOException {
		...
		for (int i = 0; i < labels.length; i++) {
			...
			selectClause.append(" ?en_label").append(i).append(" ?label")
					.append(i);
			whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri)
					.append("> ?en_label").append(i)
					.append(" FILTER langMatches(lang(?en_label").append(i)
					.append("),'en') } ");
			whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri)
					.append("> ?label").append(i)
					.append(" FILTER langMatches(lang(?label").append(i)
					.append("),'') } ");
		}

		for (int i = 0; i < descriptions.length; i++) {
			String uri = descriptions[i].trim();
			selectClause.append(" ?en_desc").append(i).append(" ?desc")
					.append(i);
			whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri)
					.append("> ?en_desc").append(i)
					.append(" FILTER langMatches(lang(?en_desc").append(i)
					.append("),'en') } ");
			whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri)
					.append("> ?desc").append(i)
					.append(" FILTER langMatches(lang(?desc").append(i)
					.append("),'') } ");
		}
	...
	public Multimap<String, String> wrapResourcePropertiesMapResultSet(
			ResultSet resultset) {
		...
		while (resultset.hasNext()) {
			...
			List<String> descriptions = getFirstBoundN(sol, vars, "desc", 1,
					true);
	...
	private List<String> getFirstBoundN(QuerySolution sol,
			List<String> varNames, String varName, int num, boolean langAware) {
